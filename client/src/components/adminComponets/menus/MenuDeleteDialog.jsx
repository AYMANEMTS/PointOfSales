...
function MenuDeleteDialog({setDeleteDialog,menuId}) {
    const queryClient = useQueryClient()
    const {setIsLoading} = useAdminContext()
    const deleteCallback = async () => {
        setIsLoading(true)
        await AdminApi.deleteMenu(menuId).then(({data}) => {
            if (data.success){
                toast.success(data.message)
                setDeleteDialog(false)
                queryClient.invalidateQueries('AdminMenus').then(() => setIsLoading(false))
            }else {
                toast.success("failed to delete menu")
                setDeleteDialog(false)
                setIsLoading(false)
                            ...
                            <div className="sm:flex sm:items-start">
                                ...
                                <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                    <h3 className="text-base font-semibold leading-6 text-gray-900"
                                        id="modal-title">Suprimer Menu !!</h3>
                                    <div className="mt-2">
                                        <p className="text-sm text-gray-500">
                                            Êtes-vous sûr de vouloir supprimer cet menu ? Cette action est irréversible.
                                        </p>
                                    </div>
                                </div>
                            </div>
